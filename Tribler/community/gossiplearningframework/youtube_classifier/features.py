...
oddities_re = re.compile(ur"(=|Â¡Â¿|Â·|\\|\^|~|â€¦|â€œ|â€�|ÃŸ|â‚¬)")
...
money_number_re = re.compile(r"[$â‚¬]-?\d{2,7}(([,\.])\d{2,7})*", re.UNICODE)
...
def load_words(install_dir='.'):
    db_dir = os.path.join(install_dir, LIBRARYNAME, 'community', 'gossiplearningframework', 'youtube_classifier')
    bad_words.update([line.strip().lower() for line in open(os.path.join(db_dir, 'db', 'badwords-adult.txt'))])
    
    reveal_words = yaml.load(open(os.path.join(db_dir, 'db', 'corp', 'word_list_dirty_words', 'reveal_questionable_words.yaml')).read())[0]['dataset']['payload'][0]['word_list']
    hate_words.update([x['word'].lower() for x in reveal_words if x['category'] == 'hate'])
    drug_words.update([x['word'].lower() for x in reveal_words if x['category'] == 'drug'])
    cult_words.update([x['word'].lower() for x in reveal_words if x['category'] == 'cult'])
    occult_words.update([x['word'].lower() for x in reveal_words if x['category'] == 'occult'])
    porn_words.update([x['word'].lower() for x in reveal_words if x['category'] == 'pornographic'])
    fwenzel_words.update([word for word in open(os.path.join(db_dir, 'db', 'corp', 'word_list_dirty_words', 'word_list-fwenzel_reporter.txt')).read().split("\n") if len(word.split()) == 1])
    
    try:
        english_vocab.update(w.lower() for w in nltk.corpus.words.words())
    except LookupError:
        nltk.download('words')
        english_vocab.update(w.lower() for w in nltk.corpus.words.words())
...
def kfold(k=10):
    ...
    from Tribler.community.gossiplearningframework.youtube_classifier.parse import load_data
